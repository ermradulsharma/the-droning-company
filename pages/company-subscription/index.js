									...
									<div className="row">
											...
											<div className="card SubscribeBox mb-2 box-shadow text-center">
												<div className="card-header">
													<div className="custom-control custom-radio">
														<input ref={monthlyPlanRef} type="radio" value={couponPlans.length ? couponPlans[0].monthly.plan_id : ''} onChange={(e)=>handSubscriptionPlan(e, 'monthly', couponPlans.length ? couponPlans[0].monthly.plan_amount : 200)} checked={planType === 'monthly' ? true : false} className="custom-control-input" id="plan_monthly" name="subscriptionPlan" />
														<label className="custom-control-label" htmlFor="plan_monthly">Monthly Plan</label>
													</div>
												</div>
												<div className="card-body">
													<h1 className="card-title pricing-card-title">${couponPlans.length ? couponPlans[0].monthly.plan_amount : 200} <small className="text-muted">/ per month</small></h1>
										...
										<div className="col-sm-6 form-group">
											<div className="card SubscribeBox mb-2 box-shadow text-center">
												<div className="card-header">
													<div className="custom-control custom-radio">
														<input type="radio" value={couponPlans.length ? couponPlans[0].annual.plan_id : ''} onChange={(e)=>handSubscriptionPlan(e, 'annual', couponPlans.length ? couponPlans[0].annual.plan_amount : 2000)} checked={planType === 'annual' ? true : false} className="custom-control-input" id="plan_annual" name="subscriptionPlan" />
														<label className="custom-control-label" htmlFor="plan_annual">Yearly Plan</label>
													</div>							
												</div>
												<div className="card-body">
													<h1 className="card-title pricing-card-title">${couponPlans.length ? couponPlans[0].annual.plan_amount : 2000} <small className="text-muted">/ Yearly</small></h1>
