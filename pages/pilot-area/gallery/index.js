...
const PhotoGallery = () => {
  ...
  const getAllImages = async (showLoading) => {
    ...
    try {
      await fetch(`${SERVER_URL}/pilot-dashboard/gallery/show/${userId}`, {
        method: "GET",
        headers: {
  ...
  const handleFiles = async (files) => {
    ...
    if (isAllFilesValid) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        data.gallery = [fields];
        listRequest.push(
          axios.post(`${SERVER_URL}/pilot-dashboard/gallery/${userId}`, data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "application/json",
              "Access-Control-Allow-Origin": "*",
  ...
  const handleDeleteImage = (id) => {
    confirm({
      ...
      .then(async () => {
        ...
        await axios
          .post(`${SERVER_URL}/pilot-dashboard/gallery/remove/${id}`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          })
