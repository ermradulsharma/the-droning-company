...
const PilotRegistration = () => {
    ...
    return (
        <div className="SignupForm paddngtb">
			<SEO 
                title={`${APPLICATION_NAME} | Pilot Registration`}
                description={''}
                siteTitle={''}
                keywords={''}
                href={currentUrl}
	        ...
	        <div className="row no-gutters row-item">
				...
				<div className="RegisterBG2 LogsBackground">
    			        <div className="SignForm"> 
    			        	<div className="alternet-access floatNone text-center">
    		          			<p>Already have an account? <Link href="/login">Log in now!</Link></p>
    		        		</div>
                            <div className="MainHeading text-center form-group">
			        		    <h1>Complete Company Account Setup</h1>
			        	    </div>
							<Formik 
								initialValues={{									
									first_name: '',
								...
								onSubmit={async fields => {
									...
									await axios.post(`${SERVER_URL}/register/company`, fields, {
										headers: {
								...
								render={({ errors, touched, handleChange, isSubmitting }) => (
									<Form className="formFill PilotRegistrationForm  paddngtb40">
										...
										<center className="paddngt">
											<button className="btn btnRegister" type="submit" disabled={isSubmitting}>Create an account <i className="fas fa-arrow-right"></i></button>
										</center>
									</Form>
								)}
							/>
